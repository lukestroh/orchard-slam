...
def generate_orchard_config(
    ...
    if total_num_trees > TREE_ID_MAX:
        raise ValueError(
            f"Total number of trees ({total_num_trees}) exceeds maximum allowed ({TREE_ID_MAX}). Please adjust parameters to reduce the total number of trees."
        )
...
def save_orchard_to_file(orchard_config: dict, output_dir: str) -> str:
    ...
    if os.path.exists(output_filepath):
        raise FileExistsError(
            f"File {file_path} already exists in {output_dir}. Please choose a different name or directory."
        )
...
def generate_all_tree_sdfs(orchard_config: dict) -> dict:
    ...
    for row_name, row in orchard_config["rows"].items():
        for i, tree_id in enumerate(row["tree_ids"]):
            ...
            tree_sdfs[tree_id] = {"sdf": generate_tree_sdf(tree_id, tree_pose), "pose": tree_pose}  # [x, y]
    return tree_sdfs
        ...
        fig.add_trace(
            go.Scatter(
                ...
                customdata=np.column_stack(
                    (row["tree_ids"], [row_name] * len(row["tree_ids"]))
                ),  # Add tree IDs and index as custom data
                hovertemplate=(
                    "%{customdata[1]}<br>"
                    "x: %{x:.2f} m<br>"
                    "y: %{y:.2f} m<br>"
